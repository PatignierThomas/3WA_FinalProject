...
export const register = async (req, res) => {
    ...
    const insertUser = "INSERT INTO users (username, birth_date, email, password, account_creation_date, account_status, role_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
    
    const hash = await bcrypt.hash(password, Number(dotenv.SALT));

    const values = [username, birth_date, email, hash, date, "ok", 1];
